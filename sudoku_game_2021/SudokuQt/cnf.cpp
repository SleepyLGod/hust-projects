...
QString ParadigmTrabverse(Root *r)
{
    Paradigm *p;
    Clause *c;
    ...
    for (p=r->first; p!=NULL; p=p->nextc)
    {
        flag=0;
        printf("第%2d行 ",i);
        answerCheckInfo += QString("第%1行 ").arg(i++);
        for (c=p->sentence; c!=NULL; c=c->nextl)
        {
            l=abs(c->literal);
            ...
            QString info3 = QString("%1真值为(%2)  ").arg(c->literal).arg(value);
            answerCheckInfo += info3;
        }
        printf("%5d\n",flag);
        QString info3 = QString("%1\n").arg(flag);
        answerCheckInfo += info3;
    }
    printf("遍历完毕！\n");
    QString info3 = QString("遍历完毕！\n");
    answerCheckInfo += info3;
    qDebug() << answerCheckInfo;
    return answerCheckInfo;
}
 ...
 * OK：解存储成功
 */
status SaveValue(ArgueValue *ValueList,int solut,int time,QString filenameInfo)
{
    int i=0;
    FILE *save;
    QString filenamesave;
    for (i=0; filenameInfo[i]!='.'; i++)
        filenamesave[i]=filenameInfo[i];
    filenamesave[i++]='.';
    filenamesave[i++]='r';
    filenamesave[i++]='e';
    filenamesave[i++]='s';
    ...
    QByteArray str = filenamesave.toLocal8Bit();
    const char *filenameSave = str.data();
